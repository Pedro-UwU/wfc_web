...
<script>
        ...
        for (let i = 0; i < height; i++) {
            for (let j = 0; j < width; j++) {
                let rect = new Rect({
                    left:
                        j * cell_width +
                        (canvas.width - width * cell_width) / 2,
                    top:
                        i * cell_height +
                        (canvas.height - height * cell_height) / 2,
                    fill: "#313131",
                    width: cell_width,
                    height: cell_height,
                    strokeWidth: 1,
    ...
    const drag_canvas = (opt) => {
        ...
        cell_x =
            Math.floor((target_x - min_x) / cell_width) * cell_width + min_x;
        cell_y =
            Math.floor((target_y - min_y) / cell_height) * cell_height + min_y;
        if (!overlay) {
    ...
    onMount(() => {
        ...
        const unssuscribe_tiles_w = tiles_width.subscribe(reset_grid);
        const unssuscribe_tiles_h = tiles_height.subscribe(reset_grid);
